...
- Dashboard camera layout: full-height roof at left, sensors/devices in the middle, TC70/SkyCam at right, compact trends and keogram along the bottom. TC70 playback uses a shared server-side HLS bridge and `/camera.php`; credentials live only in `~/.config/roof-camera/account.json` on data. Never include credentials in client configuration or Git. Camera visibility is controlled by `dashboard.showCamera`.
